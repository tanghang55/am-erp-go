...
type SalesReturnTxManager struct {
	db                  *gorm.DB
	seedLotCostResolver inventoryUsecase.SeedLotUnitCostResolver
}

func NewSalesReturnTxManager(
	db *gorm.DB,
	seedLotCostResolver inventoryUsecase.SeedLotUnitCostResolver,
) *SalesReturnTxManager {
	return &SalesReturnTxManager{
		db:                  db,
		seedLotCostResolver: seedLotCostResolver,
	}
}

func (m *SalesReturnTxManager) Run(
	ctx context.Context,
	fn func(salesUsecase.SalesReturnTransactionalDeps) error,
) error {
	return m.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		deps := salesUsecase.SalesReturnTransactionalDeps{
