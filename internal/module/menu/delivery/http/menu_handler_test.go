...
func TestCreateMenuReturnsBadRequestWhenCodeInvalid(t *testing.T) {
	gin.SetMode(gin.TestMode)

	stub := &stubMenuUsecase{err: menuusecase.ErrMenuCodeInvalid}
	handler := NewMenuHandler(stub)
	router := gin.New()
	router.POST("/api/v1/menus", handler.CreateMenu)

	payload := []byte(`{"title":"Menu","code":"菜单@001"}`)
	w := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodPost, "/api/v1/menus", bytes.NewReader(payload))
	req.Header.Set("Content-Type", "application/json")
	router.ServeHTTP(w, req)
...
func TestUpdateMenuReturnsBadRequestWhenPermissionCodeInvalid(t *testing.T) {
	gin.SetMode(gin.TestMode)

	stub := &stubMenuUsecase{err: menuusecase.ErrMenuPermissionCodeInvalid}
	handler := NewMenuHandler(stub)
	router := gin.New()
	router.PUT("/api/v1/menus/:id", handler.UpdateMenu)

	payload := []byte(`{"title":"Menu","code":"MENU_001","permission_code":"system@manage"}`)
	w := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodPut, "/api/v1/menus/12", bytes.NewReader(payload))
	req.Header.Set("Content-Type", "application/json")
	router.ServeHTTP(w, req)
